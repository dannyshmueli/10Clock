...
open class TenClock : UIControl{
    ...
    open var gradStartColor: UIColor!
    open var gradEndColor: UIColor!
    ...
    open var digitFontColor: UIColor = UIColor(white: 138/255.0, alpha: 1)
    open var digitFont = UIFont.preferredFont(forTextStyle: UIFontTextStyle.caption2).withSize(CGFloat(14))
    ...
    open var startImage: UIImage = UIImage()
    open var endImage: UIImage = UIImage()
    ...
    open func update() {
        ...
        var biggerSize = numeralInset.size
        biggerSize.width *= 1.1
        biggerSize.height *= 1.1
        numeralsLayer.occupation = (biggerSize, layer.center)
    ...
    func updateHeadTailLayers() {
        ...
        topHeadLayer.addSublayer(tImage(img: endImage, position: topHeadLayer.center))
        topTailLayer.addSublayer(tImage(img: startImage, position: topTailLayer.center))
    }
    
    func tImage(img: UIImage, position: CGPoint) -> CALayer {
        let imageLayer = CALayer()
        imageLayer.bounds.size = img.size
    ...
    func updateWatchFaceNumerals() {
        ...
        let fontSize = digitFont.pointSize
        let cgFont = CTFontCreateWithName(digitFont.fontName as CFString?,fontSize/2 ,nil)
        let startPos = CGPoint(x: numeralsLayer.bounds.midX, y: 15)
        let origin = numeralsLayer.center
        let step = (2 * M_PI) / 12
        for i in (1 ... 12){
            let l = CATextLayer()
            l.bounds.size = CGSize(width: i > 9 ? fontSize+1 : (fontSize/2)+1, height: fontSize)
            l.fontSize = fontSize
            l.alignmentMode = kCAAlignmentCenter
            l.contentsScale = UIScreen.main.scale
            //            l.foregroundColor
            l.font = cgFont
            l.string = "\(i)"
            l.foregroundColor = self.digitFontColor.cgColor
    ...
    func createSublayers() {
        ...
        self.gradEndColor = tintColor.modified(withAdditionalHue: -0.08, additionalSaturation: 0.15, additionalBrightness: 0.2)
        self.gradStartColor = tintColor
